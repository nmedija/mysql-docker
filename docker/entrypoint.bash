...
change_root_password() {
    ...
    mysql --defaults-file=/etc/mysql/entrypoint.cnf << EOF
...
create_database_user() {
    ...
    mysql "$auth" -e "CREATE USER '${username}'@'${host}' IDENTIFIED BY '${password}';"
...
create_entrypoint_user() {
    set +o xtrace
    create_database_user -uroot entrypoint "$MYSQL_ROOT_PASSWORD" "localhost" "Y"
    set -o xtrace
}

create_healthcheck_user() {
    create_database_user "--defaults-file=/etc/mysql/entrypoint.cnf" healthcheck "" "localhost" "N"
}

create_remote_admin_user() {
    set +o xtrace
    if [[ "$MYSQL_REMOTE_ADMIN_USER" != "" ]] && [[ "$MYSQL_REMOTE_ADMIN_PASSWORD" != "" ]]; then
        create_database_user "--defaults-file=/etc/mysql/entrypoint.cnf" "$MYSQL_REMOTE_ADMIN_USER" "$MYSQL_REMOTE_ADMIN_PASSWORD" "%" "Y"
    else
        errmsg "WARNING: No remote admin user has been setup."
        errmsg "         Both MYSQL_REMOTE_ADMIN_USER and MYSQL_REMOTE_ADMIN_PASSWORD values need to be set to have a remote admin user."
    fi
    set -o xtrace
}
